...
(defsection @micmac-game-theory (:title "Game Theory")
  (find-nash-equilibrium function))

(defun extremum (vector pred &key (start 0) (end (length vector)))
  "Find the first extreme value of the [START,END) subsequence of
  VECTOR and return it along with its index. The extremum is what
  would be the first element of VECTOR sorted by SORT with PRED."
    ...
    (destructuring-bind (n-rows n-cols) (array-dimensions payoff)
      (let* ((row-cum-payoff (make-array n-rows :initial-element 0))
        ...
        (dotimes (i n-iterations)
          ...
          (setq active (aref colpos
                             (nth-value 1 (extremum col-cum-payoff #'<))))
